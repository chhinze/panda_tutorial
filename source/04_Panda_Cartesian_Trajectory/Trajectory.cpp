...
TrajectoryIteratorCartesianPose::TrajectoryIteratorCartesianPose(const Trajectory &traj)
    : p_t(traj.p_t()), dp_dt(traj.dp_dt()),  dt(traj.getDt()), t_E(traj.getTEnd()), itr(0) {}

std::array<double, 16> TrajectoryIteratorCartesianPose::getCartesianPose() const {
  const Eigen::Vector6d currentPose = this->p_t.col(this->itr);

  return poseVec2HomogeneousTfArray(currentPose);
}

std::array<double, 6> TrajectoryIteratorCartesianPose::getCartesianVelocity() const{
...
void TrajectoryIteratorCartesianPose::step() { itr = itr + 1; }

double TrajectoryIteratorCartesianPose::getCurrentTime() const { return this->itr * this->dt; }
double TrajectoryIteratorCartesianPose::getEndTime() const { return this->t_E; }